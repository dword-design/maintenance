...
export default tester({
  ...
  'remove: valid': async () => {
    ...
    const output = execa(self, ['remove', 'b.txt', '-m', 'chore: remove file']) |> await |> property('stdout')
  ...
  'remove: error': () => expect(execa(self, ['remove'])).rejects.toThrow("error: missing required argument 'glob'"),
  'update-github-workflows': async () => {
    ...
    const output = execa(self, ['update-github-workflows']) |> await |> property('stdout')
    expect(output).toEqual(endent`
